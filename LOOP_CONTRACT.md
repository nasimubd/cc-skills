...
iteration: 197
status: ACTIVE
last_updated: 2026-04-24T22:40:00Z
...
- 2026-04-24 20:45 UTC — iter-174: **holiday awareness wired into session state** (7d1f0f14). Follow-up to iter-173's data-only MVP. `FCIsMarketHoliday(mkt, now)` now consumed by `computeSessionState`: (1) `isHoliday` forces `state = kSessionClosed` regardless of hour (parallels the existing `isWeekend` branch — "if (isWeekend || isHoliday) ..."); (2) both PRE-MARKET (iter-123) and AFTER-HOURS (iter-125) promotion guards add `!isHoliday` so the 15-min auction signals don't fire on a closed day; (3) the "today's-open" fast-path in secsToNext is disabled when isHoliday (falls through to cross-day advance — caveat documented: doesn't yet chain back-to-back holidays). New fixture `test_nyse_holiday_state_closed` locks the integration with 4 asserts (Thanksgiving mid-session → CLOSED; 09:25 pre-open → CLOSED not PRE; 16:05 post-close → CLOSED not AFTER; regular Friday sanity → OPEN). CLAUDE.md Known-Limitations upgraded from "data-only MVP" to "NYSE 2026 only, 13 other exchanges still unaware". Tests +1 (66 total). 66/66 pass.
- 2026-04-24 20:50 UTC — iter-175: **holiday calendar extended to LSE 2026 via per-market registry** (1e8f7d65). Refactor `HolidayCalendar.m` from a hardcoded NYSE-only check into a registry struct `FCHolidayTable { market_id, dates, count }` + array `kHolidayTables[]`. Adding an exchange's calendar is now append-only data — one array literal + one entry, zero function-body change. LSE 2026 (8 bank holidays) landed alongside NYSE 2026: covers UK-distinctive dates (Easter Monday, Spring + Summer bank holidays, Boxing Day observed Dec 28 because Dec 26 is Sat). iter-174's wiring is market-agnostic so LSE holidays automatically force CLOSED + block PRE/AFTER — no integration work. Test `test_holiday_calendar_lse` (+1) mirrors the NYSE test shape: 4 LSE-distinctive holidays flagged + cross-market negatives (LSE must NOT flag Thanksgiving, NYSE must NOT flag Easter Monday — catches registry routing bugs immediately) + regular-day negative. CLAUDE.md Known-Limitations: 2/14 markets covered; 12 still unaware; back-to-back chaining + early-close half-days still deferred. Tests 66 → 67. 67/67 pass.
- 2026-04-24 20:55 UTC — iter-176: **TSE 2026 holidays + tests/test_holidays.m split**. Data: add 18 TSE non-trading days to `HolidayCalendar.m` (Shogatsu Jan 1-2, Coming of Age Day, Golden Week Apr 29 + May 4-6 incl. May 6 furikae substitute for Sun May 3, mid-year + autumn equinox cluster, year-end Dec 31). iter-175's registry handled integration for free — no function-body change. Test-harness split: `test_session.m` was crossing the 1000-LoC file-size-guard again (first split iter-118 → `test_levers.m`). Extract all holiday fixtures (test*holiday_calendar*{nyse,lse} + test_nyse_holiday_state_closed) into new `tests/test_holidays.m` + `test_holidays.h` — function bodies move, main()-wiring stays. New fixture `test_holiday_calendar_tse` (+1) covers 7 positive asserts (per holiday category) + 4 cross-market negatives + regular-day negative. test_session.m 987 → 875 LoC (back under guard). Tests 67 → 68. CLAUDE.md: test-count + Known-Limitations refreshed — 3/14 markets covered, 11 still unaware; back-to-back chaining + early-close half-days remain deferred.
- 2026-04-24 21:00 UTC — iter-177: **back-to-back holiday chaining in next-open countdown** (bc36d935). Closed the "caveat: doesn't yet chain back-to-back holidays" bullet from iter-174. Replace computeSessionState's hardcoded weekend-skip loop (`while (nextWeekday == 1 || nextWeekday == 7) addDays++`) with an NSDate-based loop that advances through candidate days and calls `FCIsMarketHoliday` alongside the weekend check. Each candidate is constructed via `[cal dateByAddingUnit:NSCalendarUnitDay ...]` so DST transitions and market-local weekday computation stay correct. 14-day safety cap. New fixture `test_holiday_chains_through_weekend` (+1) validates two real scenarios with 60s tolerance: LSE Thu 2026-12-24 18:00 → Dec 29 Tue 08:00 (chains Xmas Fri + Sat + Sun + Boxing Day Mon = 4 closed days + 5.5h to next open = 110h = 396000s); NYSE Fri 2026-12-25 18:00 → Dec 28 Mon 09:30 (Xmas + weekend = 63.5h = 228600s). Both fixtures would have FAILED with iter-176's loop — the test confirms the correctness gap existed and is now closed. Known Limitations: chaining caveat removed; only 11-markets-no-data + early-close-half-days remain. Tests 68 → 69.
- 2026-04-24 21:05 UTC — iter-178: **HKEX 2026 holidays (first lunar-calendar market)** (4fc65917). Add 15 HKEX non-trading days to `HolidayCalendar.m` — first calendar with lunar-calendar holidays whose Gregorian dates shift year-by-year: Lunar New Year 3-day cluster (Feb 17/18/19), Buddha's Birthday (May 25), Dragon Boat (Jun 19), Mid-Autumn (Sep 25), Chung Yeung (Oct 19). Plus Western (Good Friday, Easter Mon + Ching Ming coincident) and civic dates (Labour Day, HKSAR Establishment, National Day, Xmas, Boxing observed). iter-175 registry handled integration for free. New fixture `test_holiday_calendar_hkex` (+1) = 10 positive asserts + 4 cross-market negatives + regular-day negative. Authoring the test surfaced two real insights documented in code comments: (a) **genuine multi-market coincidence** — Jun 19 2026 is BOTH NYSE Juneteenth and HKEX Dragon Boat; both calendars correctly flag it, no negative test possible; (b) **TZ-conversion pitfall** — HK-noon of LNY Feb 17 lands on NY-evening of Feb 16 Presidents' Day after UTC offset, so cross-market negatives must use dates that don't overlap NY holidays after conversion. Known Limitations: coverage 3/14 → 4/14, 10 still unaware. Tests 69 → 70.
- 2026-04-24 21:10 UTC — iter-179: **XETRA + Euronext paired via shared TARGET2 data array** (a8601042). Biggest single-iter coverage jump so far (4/14 → 6/14) by exploiting the TARGET2-settlement-calendar identity of Frankfurt and Paris/Amsterdam/Brussels/Lisbon/Dublin exchanges in 2026: both share the same 5 full-day closures (Jan 1, Good Fri, Easter Mon, May 1, Dec 25 — Dec 26 Boxing Day falls Sat 2026 → weekend handles). Implementation: one `kTARGET2_2026Holidays[]` array referenced by TWO registry entries (`xetra`, `euronext`). Zero data duplication — the iter-175 registry design accommodates N-markets-share-one-array with zero extra code. New fixture `test_holiday_calendar_target2` (+1) = 5 dates × 2 markets (10 positive checks) + Dec 26 intentionally-not-in-array sanity + 3 cross-market negatives (NYSE Thanksgiving, TSE Shogatsu, HKEX LNY) + regular-day negative. Known Limitations: coverage 4/14 → 6/14; 8 markets still unaware. Tests 70 → 71.
- 2026-04-24 21:15 UTC — iter-180: **ASX 2026 (first Oceania coverage)** (a83a8d02). Add 8 ASX 2026 non-trading days: New Year's, Australia Day (Jan 26), Good Friday, Easter Monday, Easter Tuesday (ASX-distinctive — one of the only major exchanges closing a 4-day Easter long weekend), King's Birthday (2nd Mon of Jun = Jun 8), Christmas, Boxing Day observed Dec 28. ANZAC Day Apr 25 2026 is Saturday — ASX does NOT observe a Mon substitute when on weekend, and the test locks Apr 27 Mon is NOT flagged. New fixture `test_holiday_calendar_asx` (+1) = 4 positive + 3 NYSE cross-negatives + 2 ASX-cross-negatives + ANZAC no-substitute-lock + regular-day negative. Coverage 6/14 → 7/14, 7 markets still unaware. Tests 71 → 72.
- 2026-04-24 21:20 UTC — iter-181: **TSX 2026 (Canadian calendar)** (37bf7128). Add 10 TSX non-trading days: New Year's, Family Day (3rd Mon Feb = Feb 16 — Ontario), Good Friday, Victoria Day (Mon on/before May 24 = May 18), Canada Day (Jul 1), Civic Holiday (1st Mon Aug = Aug 3), Labour Day (coincides w/ NYSE), Canadian Thanksgiving (2nd Mon Oct = Oct 12 — month earlier than US), Xmas, Boxing observed. Notable — TSX does NOT close Easter Monday (distinct from LSE/TARGET2/HKEX/ASX). New `test_holiday_calendar_tsx` (+1) = 6 positive asserts + explicit Easter-Monday-not-flagged lock + 3 NYSE negatives (Cdn Thanks/Canada Day/Civic) + US-Thanks-not-on-TSX lock + regular-day negative. Coverage 7/14 → 8/14, 6 still unaware. Tests 72 → 73.
- 2026-04-24 21:25 UTC — iter-182: **SIX Swiss Exchange 2026** (f744e6f4). Add 10 SIX non-trading days: New Year's, Berchtold's Day (Jan 2 Swiss-specific), Good Friday, Easter Mon, Labour Day, Ascension (May 14 = Easter Sun + 40d), Whit Mon (May 25 = Easter Sun + 50d), **Xmas Eve Dec 24 FULL closure**, Xmas, **NYE Dec 31 FULL closure**. SIX is one of the only exchanges where Dec 24 and Dec 31 are full-day closures rather than half-day sessions — test cross-validates that XETRA (TARGET2) does NOT flag these dates. Aug 1 Swiss National Day falls Saturday — no Mon substitute. New `test_holiday_calendar_six` (+1) = 5 positive asserts + 5 XETRA cross-negatives + Thanksgiving negative + Aug 3 no-substitute lock + regular-day negative. Europe region now fully covered: LSE + XETRA + Euronext + SIX. Coverage 8/14 → 9/14, 5 still unaware (JSE, B3, KRX, NSE, SSE). Tests 73 → 74.
- 2026-04-24 21:30 UTC — iter-183: **SSE 2026 (first Chinese market, dual Golden Week)** (bd42e5e0). Most complex calendar yet — 16 dates covering two 7-day Golden Week clusters: Spring Festival Feb 17-23 (5 weekday closures + Feb 21-22 weekend) + National Day Oct 1-7 (5 weekday closures + Oct 3-4 weekend). Plus Qingming Apr 6, Labour cluster May 1/4/5, Dragon Boat Jun 19 (lunar overlap w/ HKEX), Mid-Autumn Sep 25 (lunar overlap w/ HKEX). New `test_holiday_calendar_sse` (+1) = 10 positive asserts + 2 HKEX shared-lunar-date positives (cross-validate both calendars flag Dragon Boat + Mid-Autumn on same Gregorian date) + 2 cross-negatives + regular-day negative. New Known-Limitation bullet: China's "make-up trading Saturdays" (补班) are NOT modelled — SSE trades on compensatory working Saturdays but weekend branch reports CLOSED. Coverage 9/14 → 10/14, 4 still unaware (JSE, B3, KRX, NSE). Tests 74 → 75.
- 2026-04-24 21:35 UTC — iter-184: **KRX 2026 (Korean substitute-holiday mechanics)** (7b101af0). Add 15 KRX non-trading days featuring Korean substitute-holiday law — when specified public holidays fall Sat/Sun, next weekday becomes a compensatory holiday. 2026 weekend-shifted dates: Mar 2 Mon (Independence Movement Mar 1 Sun), Aug 17 Mon (Liberation Aug 15 Sat), Sep 28 Mon (Chuseok Day 3 Sep 26 Sat), Oct 5 Mon (Foundation Oct 3 Sat). Multi-day clusters: Seollal Eve + Days 1-2 (Feb 16-18) and Chuseok Days 1-3 (Sep 24-26). KRX also closes Dec 31 as year-end ceremony (폐장일 — Korean-specific). **Sep 25 2026 is a TRIPLE-market lunar coincidence**: Chuseok Day 2 (KRX) = Mid-Autumn (HKEX) = Mid-Autumn (SSE); the test cross-validates all 3 markets flag this date. New `test_holiday_calendar_krx` (+1) = 12 positive asserts + triple-market lunar lock + cross-negative + regular-day negative. Coverage 10/14 → 11/14, 3 still unaware (JSE, B3, NSE). Tests 75 → 76.
- 2026-04-24 21:40 UTC — iter-185: **NSE 2026 (most diverse calendar mix)** (dd185b75). Add 14 NSE non-trading days spanning 5 religious / cultural traditions: (1) Gregorian civic (Republic Day, Maharashtra Day, Gandhi Jayanti); (2) Christian (Good Friday, Xmas); (3) Hindu lunar — Holi/Dhuleti, Ram Navami, Ganesh Chaturthi, Dussehra, Diwali 2-day cluster (Laxmi + Govardhan); (4) Jain (Mahavir Jayanti); (5) Sikh (Guru Nanak); (6) Muslim (Bakri Id). Lunar-derived dates marked "(approximate)" — fixture-locked best-effort. NSE does NOT substitute weekend-coincident holidays — Aug 15 2026 is Sat and test locks Mon Aug 17 is NOT flagged. New `test_holiday_calendar_nse` (+1) = 5 Gregorian-fixed + 5 lunar-derived + Aug 17 no-substitute lock + 3 NYSE cross-negatives + regular-day. Coverage 11/14 → 12/14, only JSE + B3 remain. Tests 76 → 77.
- 2026-04-24 21:45 UTC — iter-186: **JSE + B3 paired; 14/14 holiday-awareness MILESTONE COMPLETE** (5e13b1d0). Add JSE (10 dates — Family Day / Easter Mon, Freedom Day, Youth Day, Heritage Day, Reconciliation, civic dates; SA substitute rule lands Women's Day Aug 10 Mon for Aug 9 Sun) + B3 (12 dates — Carnival Mon+Tue cluster Feb 16-17, Corpus Christi Easter+60d, Tiradentes, Independence, Aparecida, All Souls', Black Awareness, civic dates) in single iter. Test authoring surfaced two more genuine multi-market coincidences worth documenting: Feb 16 2026 = B3 Carnival Mon AND NYSE Presidents' Day; Sep 7 2026 = B3 Independence Day AND NYSE Labor Day — cross-market negatives use B3-only Carnival Tue / Tiradentes / Corpus Christi instead. New `test_holiday_calendar_jse` + `test_holiday_calendar_b3` (+2 fixtures, ~20 asserts). **Coverage 12/14 → 14/14 — campaign milestone reached** (campaign began iter-173 with NYSE-only data; 13 iters later all 14 markets in the roster have full 2026 calendars). Holiday-data caveat removed from CLAUDE.md Known Limitations; only half-day + China make-up Saturdays + lunar-approximate caveats remain. Tests 77 → 79.
- 2026-04-24 21:50 UTC — iter-187: **v1.11.0 release consolidation** (76818da6). Ship the holiday-awareness campaign as a version-bumped release. plugin.json 1.10.0 → 1.11.0; Info.plist CFBundleShortVersionString + CFBundleVersion (172 → 187). 15 iters since v1.10 (iter-172 → iter-187 — longest release span so far, justified by the functional campaign scope). plugin.json description rewritten with v1.11 headline: "full 2026 holiday-awareness across all 14 exchanges" — lists campaign wins (FCIsMarketHoliday wired, back-to-back chaining correct, substitute-holiday rules auto-shift, multi-religious coverage), cites the 4 genuine multi-market coincidences (Feb 16 / Jun 19 / Sep 7 / Sep 25), documents 3 remaining caveats. Build clean, validator clean, all 79 tests pass, binary ~220 KB. Release marks the end of the iter-173 → iter-186 data-expansion campaign and opens space for next campaign theme (half-day sessions, UX polish, or fresh aesthetic expansion).
- 2026-04-24 21:55 UTC — iter-188: **half-day calendar MVP** (just committed). First post-v1.11.0 feature iter — opens the half-day-session campaign with same data-only-MVP pattern as iter-173's holiday launch. New `Sources/data/HalfDayCalendar.{h,m}`: FCHalfDayEntry struct `{ISO date, close_h, close_m}`, per-market array pattern parallel to iter-175 registry, `FCIsMarketHalfDay(mkt, date, &closeH, &closeM)` lookup (out-params optional). NYSE 2026 MVP data: Black Friday Nov 27 + Xmas Eve Dec 24, both 13:00 ET close. New `test_halfday_calendar_nyse` (+1) = 8 asserts — close-time populated, nil-out-param probe, regular-day negative, Thanksgiving not-flagged-half-day (full-holiday and half-day are disjoint sets), other-market-no-data, nil-safety. Not-yet-wired into computeSessionState — caveat updated to "data layer shipped, wiring pending". Tests 79 → 80.
- 2026-04-24 22:00 UTC — iter-189: **wire FCIsMarketHalfDay into computeSessionState** (e3217f2e). Integration iter parallels iter-174 (holiday wiring). Small edit in MarketSessionCalculator.m: after computing regular closeMins, probe `FCIsMarketHalfDay(mkt, now, &h, &m)` and override closeMins + set hasLunch=NO when half-day. Lunch suppression is conservative: NYSE has no lunch anyway, but TSE/HKEX/SSE half-days (when added) shouldn't inherit lunch semantics since a ~4h half-session wouldn't contain one. New fixture `test_nyse_halfday_state_closed` (+1): Black Friday 10:00 ET → OPEN / Black Friday 14:00 ET → CLOSED (past 13:00 early close) / Xmas Eve same / regular Fri 14:00 → OPEN sanity. Pre-integration the 14:00 asserts would have FAILED — test is direct proof of the correctness gap iter-189 closes. NYSE half-days now fully wired. 13 other markets still await half-day data but will integrate for free via the registry pattern. Known Limitations refreshed. Tests 80 → 81.
- 2026-04-24 22:05 UTC — iter-190: **half-day extended to LSE + TARGET2 pair (XETRA+Euronext)** (1b3324ec). Add LSE 2026 (Dec 24 + Dec 31 at 12:30 London) + shared kTARGET2_2026HalfDays[] (Dec 24 + Dec 31 at 14:00 local) referenced by both xetra + euronext registry entries — mirrors iter-179 full-holiday TARGET2 dedup. iter-189's computeSessionState wiring is market-agnostic so the new markets get early-close for free. New `test_halfday_calendar_lse_and_target2` (+1): 4 close-time asserts + cross-dedup sanity (XETRA flags Euronext's Dec 31, vice versa) + disjoint-set check (LSE Dec 24 half-day NOT full-holiday) + 3 regular-day negatives. Half-day coverage 1/14 → 4/14. Tests 81 → 82.
- 2026-04-24 22:10 UTC — iter-191: **half-day extended to HKEX + TSX** (2711f980). HKEX is the first lunch-bearing market with half-day data — its regular schedule is 09:30-12:00 / 13:00-16:00 with 12:00-13:00 lunch, and half-day early close lands AT lunch start. iter-189's `hasLunch = NO when isHalfDay` override becomes load-bearing: test fixture explicitly asserts 12:30 on Xmas Eve half-day returns CLOSED (not LUNCH) — without the guard this would have been LUNCH, a silent correctness bug. Added HKEX 3 dates (LNY Eve Feb 16 + Xmas Eve + NYE, all 12:00 HKT) + TSX Dec 24 only (13:00 ET — TSX trades Dec 31 full day, distinct from LSE's half-day; test locks this Canadian quirk). New `test_halfday_calendar_hkex_and_tsx` (+1) = 4 HKEX close-time asserts + 12:30-Xmas-Eve-integration-lock + 10:00-Xmas-Eve-OPEN + TSX Dec 24 + TSX-Dec-31-NOT-flagged. Half-day coverage 4/14 → 6/14. Tests 82 → 83.
- 2026-04-24 22:15 UTC — iter-192: **half-day JSE + ASX paired** (87e44910). Fills Africa + Oceania region gap. JSE Dec 24 closes 12:00 SAST; ASX Dec 24 closes 14:10 AEDT. Neither has Dec 31 half-day (both trade full Dec 31, distinct from LSE). First draft tripped the 1000-LoC file-size guard on test_holidays.m (would have been 1003 lines); tightened the test to essentials (close-time asserts + Dec-31-NOT-flagged locks). New `test_halfday_calendar_jse_and_asx` (+1). Half-day coverage 6/14 → 8/14. Tests 83 → 84.
- 2026-04-24 22:20 UTC — iter-193: **extract half-day tests to tests/test_halfdays.m** (just committed). Second test-split of the campaign (first: iter-176 holiday extraction from test_session.m). test_holidays.m was at 982 LoC — iter-192 had to tighten its test specifically to stay under the 1000-LoC guard, and the next half-day iter couldn't grow without hitting the guard. Move the 5 half-day fixtures (iter-188 to iter-192) to their own file. Local helpers duplicated (halfdayDateAt + halfdayStateName + ASSERT_HDSTATE macro) — same local-helpers pattern as test_holidays.m. Makefile TEST_SOURCES updated; test_session.m imports test_halfdays.h. test_holidays.m 982 → 774 LoC (~225 LoC headroom); test_halfdays.m new @ 208 LoC (~790 LoC runway). All 84 tests pass (no logic change, just relocation). Next half-day iters can grow freely.
- 2026-04-24 22:25 UTC — iter-194: **real user bug report — "Transparency failed to work"** (52bc6440). User interrupted the autonomous loop to report the global Canvas Opacity menu feeling broken. Root-cause diagnosis (no iter-188+ change caused it; pre-existing since iter-90): the 3-tier opacity resolver (per-segment → CanvasOpacity → theme alpha) is correct in isolation, but TWO entry-point UX bugs let stale per-segment keys pin segments regardless of the global slider. Fix 1 — `setCanvasOpacity:` now removes LocalOpacity/ActiveOpacity/NextOpacity alongside writing CanvasOpacity, so the global slider acts as override-all. Fix 2 — `activateProfile:` clears ALL managed keys first, then applies the new profile's values; previous impl used `if (val != nil) set` which leaked stale top-level keys across profile switches. No new tests — bug manifests in rendering layer that the data-layer unit-test harness doesn't cover. Manual verification: user's stale LocalOpacity=0.75 + NextOpacity=0.75 cleared, fresh binary relaunched. Per-segment opacity (via dedicated per-segment menus) still works as before — those paths write their own keys and don't clear anything. 84 tests still pass, validator clean.
- 2026-04-24 22:30 UTC — iter-195: **Aurora theme (first aesthetic iter since iter-172's v1.10 release)** (71cfbf47). Pivot from the iter-173 → iter-194 session/holiday/half-day data push back toward the contract's aesthetic Core Directive. Add "Aurora" theme — cyan-green fg (0.35, 0.92, 0.82) on deep indigo-black bg (0.03, 0.05, 0.12), alpha 0.45. Aurora borealis visual motif, cool winter-night mood distinct from existing 28 themes (warmer than nord/tokyo_night; more saturated than mint_dark; bluer than lavender_dream). 29th theme total. Invariant test `test_theme_catalog_invariants` hard-asserts kNumThemes — bumped 28 → 29, per-theme RGB + alpha range checks pass unchanged. 84 tests still pass. CLAUDE.md theme roster history updated: originals 10 + iter-32 (+10) + iter-92 (+5) + iter-132 (+2) + iter-169 Carnival + iter-195 Aurora.
- 2026-04-24 22:35 UTC — iter-196: **Borealis Quick Style (paired with iter-195 Aurora)** (9f7d6821). Repeats the iter-169 Carnival-theme → iter-170 Samba-Quick-Style narrative-pairing pattern. 14th Quick Style mood — aurora theme + squircle + halo shadow + thin weight + wide letter-spacing + loose line-spacing + middot separator. Evokes auroral light dancing over arctic dark. Distinct from existing cool moods: Glacier is desaturated (nord), Midnight is dense, Borealis is the luminous cool counterpart. `test_quick_styles_invariants` verifies bundle keys are valid presets — Borealis's values all land in existing families, passes. 84 tests pass. CLAUDE.md Quick Style table + Sources tree updated.
- 2026-04-24 22:40 UTC — iter-197: **Progress-bar glyph catalog +2 (14 total)** (2727fe09). Third expansion on this lever (iter-27: 6, iter-91: +4, iter-131: +2, iter-197: +2). New pairs — `waves ≋ ∼` (flowing tilde texture, fluid horizontal motion) + `chevrons ❯ ›` (stacked directional arrows, more urgent than the existing single-triangle arrows). Three touchpoints as usual: fcGlyphsForStyle dispatcher +2 if-branches, ProgressBarStyle submenu +2 entries in semantic-grouping order (chevrons next to arrows, waves next to braille), test_progress_bar_glyph_styles +2 end-to-end test cases verified through buildProgressBar rendering. 84 tests still pass. CLAUDE.md ProgressBarStyle row updated (12 → 14).
