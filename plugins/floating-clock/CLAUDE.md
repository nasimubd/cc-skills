...
- **Partial holiday awareness (NYSE 2026 only)**. iter-173 shipped `Sources/data/HolidayCalendar.{h,m}` with a hardcoded NYSE-2026 holiday list and `FCIsMarketHoliday(mkt, date)` lookup; iter-174 wired the result into `computeSessionState` so NYSE on Thanksgiving / Christmas / etc. correctly reports CLOSED and blocks PRE-MARKET / AFTER-HOURS promotions. Caveats still in play: (1) the 13 other exchanges have no holiday data — TSE Shogatsu, LSE bank holidays, JSE, B3 Carnival, HKEX Lunar New Year etc. still read OPEN on their respective holidays; (2) when today is a NYSE holiday, the countdown still points at tomorrow's regular open — if tomorrow is also a holiday (e.g. Dec 25→26 double close), the shown boundary will be stale until the next tick past midnight. Multi-market calendars + back-to-back holiday chaining land in subsequent iters.
