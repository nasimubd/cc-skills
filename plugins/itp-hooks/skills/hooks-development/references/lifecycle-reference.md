<!-- SKILL-PLUGIN-ROOT-OK: reference documentation about this very variable -->
...
- `CLAUDE_PLUGIN_ROOT` — Plugin root directory. **IS** set for hooks declared in a plugin's own `hooks/hooks.json` (and `${CLAUDE_PLUGIN_ROOT}` in that file's `command` is substituted at load time). It is **NOT** set for hooks hand-written into `settings.json` (not plugin-associated), and **NOT** available to the Bash tool, so a `SKILL.md` must never reference it — corrected 2026-08-05; see Common Pitfalls below
